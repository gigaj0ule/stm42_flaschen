...
_IO32(A); _IO32(B); _IO32(C);
  ...
  // SPI Definitions
  #define BOARD_SPI1_MOSI_PIN PIN_SPI_MOSI
  #define BOARD_SPI1_SCK_PIN PIN_SPI_SCK
  ...
  _DEFPIN_ARM(PA12, 12, A);	// USB DP
  _DEFPIN_ARM(PA11, 11, A);	// USB DM
  ...
  _DEFPIN_ARM(PB12, 12, B); // LED Blackpill
  // Right side
  _DEFPIN_ARM(PC13, 13, C); // LED Bluepill
  _DEFPIN_ARM(PC14, 14, C);
  _DEFPIN_ARM(PC15, 15, C);
  _DEFPIN_ARM(PA0, 0, A); // A0
  _DEFPIN_ARM(PA1, 1, A);	// A1
  _DEFPIN_ARM(PA2, 2, A);	// A2
  _DEFPIN_ARM(PA3, 3, A); // A3
  _DEFPIN_ARM(PA4, 4, A); // A4
  _DEFPIN_ARM(PA5, 5, A); // A5
  _DEFPIN_ARM(PA6, 6, A); // A6
  _DEFPIN_ARM(PA7, 7, A); // A7
  _DEFPIN_ARM(PB0, 0, B); // A8
  _DEFPIN_ARM(PB1, 1, B);
  _DEFPIN_ARM(PB10, 10, B);
  // Other
  _DEFPIN_ARM(PB2, 2, B); // BOOT1
  _DEFPIN_ARM(PA13, 13, B); // SWDI0
  _DEFPIN_ARM(PA14, 14, B); // SWCLK

  // This must be a literal
  #define MAX_PIN NUM_DIGITAL_PINS
